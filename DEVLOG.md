...
- **症狀：** `AttributeError` 或 transform 執行失敗
...
- **症狀：** `UserWarning: The given NumPy array is not writable`（出現在 `acdc_dataset.py:97`）
...
- **症狀：** `Expected all tensors to be on the same device`
...
- **症狀：** 聯邦聚合後全局模型 Dice 接近 0，但各 client 本地 Dice 正常
...
- **症狀：** 20 輪訓練後 Dice 全程維持在 0.025，完全不收斂
...
- **症狀：** `UnboundLocalError: cannot access local variable 'ndarrays_to_parameters'`
